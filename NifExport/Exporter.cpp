	...
	if (!Exporter::mSelectedOnly)
	{
		if (IsOblivion() || IsFallout3() || IsSkyrim() || IsFallout4())
		{
			if (mSkeletonOnly)
			{
				...
				if (IsSkyrim()) {
					//WIP, read values from varius versions if exists
					BoneLOD* info = new BoneLOD();
					info->boneName = "NPC Root [Root]";
					info->distance = 2048;
					BSBoneLODExtraDataRef bslod = CreateNiObject<BSBoneLODExtraData>();
					bslod->SetName("BSBoneLOD");
					std::vector<BoneLOD>* infovec = new std::vector<BoneLOD>();
					infovec->push_back(*(info));
					bslod->SetBonelodInfo(*(infovec));
					root->AddExtraData(DynamicCast<NiExtraData>(bslod));
				}
				...
				if (IsSkyrim()) {
					//need to experiment, unknown value
					NiIntegerExtraDataRef skeletonFlags = CreateNiObject<NiIntegerExtraData>();
					skeletonFlags->SetName("SkeletonID");
					skeletonFlags->SetData(883515280);
					root->AddExtraData(DynamicCast<NiExtraData>(skeletonFlags));
